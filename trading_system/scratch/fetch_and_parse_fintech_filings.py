...
with open('scratch/valid_fintech_companies.json', 'r') as f:
    companies = json.load(f)

print(f"Loaded {len(companies)} valid fintech companies.")

selected_tickers = [
    'CPSS', 'OPRT', 'LPRO', 'FINW', 'SNFCA', 'RM', 'MVBF', 'BRBS', 
    'USIO', 'TPVG', 'SEVN', 'GHI', 'CZNC', 'HNVR', 'AVBH', 'BWFG', 
    'USCB', 'GBFH', 'JMSB', 'FBLA', 'FUSB', 'NKSH', 'PKBK', 'FCCO'
]
...
for ticker in selected_tickers:
    ...
    for i in range(len(forms)):
        ...
        if form in ['8-K', '8-K/A'] and '2025-07-23' <= fdate <= '2026-07-23':
            ...
            if company_filings_count >= 3:
                break

print(f"\nTotal fintech filings fetched and parsed: {len(results)}")

with open('scratch/parsed_fintech_8k_filings.json', 'w') as f:
