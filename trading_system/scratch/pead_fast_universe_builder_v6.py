...
def process_and_eval_mcap(item):
    ticker = item['ticker']
    ...
    # 2. Market Cap Check via yfinance
    try:
...
def main():
    ...
    for idx in range(0, len(tickers_list), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            futures = [executor.submit(process_and_eval_mcap, item) for item in chunk]
        ...
        time.sleep(0.60) # Pacing SEC rate limits (<10 req/sec)
