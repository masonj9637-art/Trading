...
## Corpus Check
- 286 files · ~689,832 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1316 nodes · 1936 edges · 201 communities (153 shown, 48 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 51 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `09b19eb6`
...
5. `TradingSystem` - 22 edges
6. `AlpacaDataFetcher` - 21 edges
7. `run_curator_export_step()` - 20 edges
8. `score_unscored_theses()` - 19 edges
9. `SECForm4Parser` - 19 edges
10. `run_director_step()` - 18 edges
...
## Communities (201 total, 48 thin omitted)
...
Cohesion: 0.11
Nodes (12): DeepOFIAgent, DataFrame, Series, DEPRECATED / UNUSED MODULE This agent uses a Volume-Weighted Return proxy to…, Since real L2 Limit Order Book data isn't available, we use a Volume-Weighted…, AdaptiveKalmanAgent, DataFrame, Series (+4 more)
...
Cohesion: 0.10
Nodes (39): get_all_thesis_scores(), Saves an immutable audit thesis record into thesis_ledger. Returns True if…, Retrieves all records from thesis_scores JOINed with thesis_ledger metadata., save_thesis_ledger_entry(), add_trading_days(), calculate_cost_adjusted_return(), generate_scoring_report(), get_alpaca_price() (+31 more)

### Community 4 - "test_sources.py"
Cohesion: 0.22
Nodes (12): fetch_currents_items(), Any, Fetches tech & industry news items from Currents API matching keywords. :param…, fetch_uspto_items(), Any, Fetches patent application data filtered by CPC classification codes from the…, Unit tests for research_scanner.sources package (arXiv, USPTO, Currents)., test_fetch_currents_items_missing_key() (+4 more)
...
Cohesion: 0.29
Nodes (3): EnsembleAgent, Executes Mean-Variance Thompson Sampling (MVTS) with Combinatorial Adaptive…, Updates the MVTS priors using CADTS geometric discounting based on continuous…
...
### Community 12 - "db.py"
Cohesion: 0.12
Nodes (32): Connection, compute_item_hash(), get_all_candidates(), get_all_ledger_entries(), get_all_trades(), get_db_connection(), get_unscored_ledger_entries(), is_item_fetched() (+24 more)

### Community 13 - "resolve_peak_nav"
Cohesion: 0.50
Nodes (4): Resolves the peak NAV using Redis with a disk-backed fallback., resolve_peak_nav(), Simulate Redis being unavailable (None or throwing exception) or returning no…, test_drawdown_persistence_redis_unavailable_disk_fallback()
...
### Community 15 - "fetch_arxiv_items"
Cohesion: 0.18
Nodes (8): fetch_arxiv_items(), Any, arXiv API data fetcher using feedparser., Fetches recent papers from arXiv API for specified categories using feedparser.…, Currents API news data fetcher., Data source modules for research_scanner (arXiv, USPTO, Currents API)., USPTO Patent data fetcher using USPTO Open Data Portal (ODP) API., test_fetch_arxiv_items_success()
...
Cohesion: 0.28
Nodes (7): Main scan entrypoint for research_scanner. Runs one full fetch -> deduplicate…, Executes one full fetch + triage cycle across all configured data sources.…, run_scan_cycle(), fixture, Integration tests for research_scanner.scan entrypoint., temp_db_path(), test_run_scan_cycle_end_to_end()

### Community 20 - "test_export_to_obsidian.py"
Cohesion: 0.17
Nodes (21): get_fetched_item_by_id(), get_unconsumed_items(), Saves a newly fetched item into the fetched_items table. Returns True if…, Retrieves all fetched items that have not yet been consumed by the curator., Retrieves a fetched item by its database ID., save_fetched_item(), export_from_curator_decisions(), Reads a JSON file containing curator decisions and exports the corresponding… (+13 more)
...
Cohesion: 0.16
Nodes (15): main(), Terminal CLI dashboard for research_scanner. Lists unreviewed candidate items…, Fetches and displays unreviewed candidates from the database in a terminal-…, render_dashboard(), init_db(), Initializes the SQLite database schema if tables do not exist., Entrypoint to process director requests. Polls the director_requests table and…, run_process_requests() (+7 more)

### Community 34 - "config.py"
Cohesion: 0.31
Nodes (6): Configuration settings for research_scanner. All settings (score threshold,…, apply_director_output(), director_apply.py Applies Director's JSON output deterministically., Discord REST notification dispatcher for research_scanner., Sends a plain string message to Discord via REST POST. :param content: The…, send_discord_message()

### Community 35 - "send_discord_notification"
Cohesion: 0.24
Nodes (10): Exception, Any, Sends a Discord notification via direct REST POST to the specified channel.…, send_discord_notification(), Unit tests for research_scanner.notifier module., test_send_discord_notification_failure(), test_send_discord_notification_missing_credentials(), test_send_discord_notification_success() (+2 more)
...
Cohesion: 0.12
Nodes (10): AlphaIsolator, DataFrame, Strips out the autoregressive momentum component to calculate the adjusted…, PCAOrthogonalizer, DataFrame, Takes a DataFrame of returns (dates as index, assets as columns). Extracts…, DataFrame, Aligns a long-format DataFrame to a continuous time index, padding missing… (+2 more)
...
Cohesion: 0.08
Nodes (41): get_director_prompt(), main(), Any, director_run.py Director execution runner module for research_scanner. Invokes…, Validates that parsed JSON is a dict matching Director's expected schema.…, Updates the shared timestamp file at timestamp_path with the current ISO 8601…, Invokes Director via agy CLI, parses and validates output JSON, writes…, Constructs Director prompt by loading vault-index.md, current-priorities.md,… (+33 more)
...
### Community 79 - "run_curator_export_step"
Cohesion: 0.14
Nodes (19): Step 3: Invokes Curator via agy CLI in headless mode, parses the JSON decision…, run_curator_export_step(), patch, Verify successful Curator CLI run with markdown codeblock fences inside inner…, Verify quota failure is detected and logged distinctly, skipping export., Verify generic CLI error logs an ERROR and skips export without quota message., Verify empty stdout logs error and Curator CLI stderr in full., Verify unparseable envelope JSON output logs an ERROR, raw output, and stderr. (+11 more)

### Community 94 - "test_heartbeat_check.py"
Cohesion: 0.20
Nodes (14): check_heartbeat(), get_timestamp_age_seconds(), Any, Calculates age in seconds from dt to now. Handles naive vs aware datetimes…, Reads last_success.json and verifies that scan_and_curator (<= 6h) and director…, Updates the shared last_success.json timestamp file for the specified key,…, update_last_success(), Unit tests for heartbeat_check.py and run_daemon last_success timestamp updates. (+6 more)
...
### Community 104 - "main"
Cohesion: 0.21
Nodes (11): main(), setup_logging(), Unit tests for director_apply.py module., Verify director_apply handles direct JSON files., Verify director_apply handles agy JSON envelope format., Verify director_apply handles agy JSON envelope format with markdown codeblock…, Verify director_apply exits non-zero if envelope response field is invalid JSON., test_director_apply_agy_envelope_json() (+3 more)

### Community 108 - "get_agy_executable_path"
Cohesion: 0.20
Nodes (10): get_agy_executable_path(), Returns the full absolute path to the agy executable. Priority: 1.…, Verify successful Curator CLI run with agy JSON envelope format., Verify FileNotFoundError handler logs the exact attempted path., Verify get_agy_executable_path returns path discovered by shutil.which when…, Verify get_agy_executable_path honors AGY_EXECUTABLE_PATH environment variable., test_get_agy_executable_path_env_override(), test_get_agy_executable_path_which_discovery() (+2 more)
...
### Community 132 - "heartbeat_check.py"
Cohesion: 0.29
Nodes (7): main(), parse_iso_timestamp(), Heartbeat monitor for research_scanner scheduled jobs. Checks…, Registers or updates the Task Scheduler entry for heartbeat_check.py to run…, Parses an ISO 8601 timestamp string into a datetime object. Returns None if…, register_task_scheduler(), test_parse_iso_timestamp()

### Community 136 - "fetch_openalex_items"
Cohesion: 0.29
Nodes (7): fetch_openalex_items(), Any, OpenAlex API data fetcher for global research papers, preprints, and patents.…, Reconstructs clean text summary from OpenAlex's abstract_inverted_index. :param…, Fetches recent patents, preprints, and research items from OpenAlex matching…, reconstruct_abstract(), test_fetch_openalex_items_success()

### Community 137 - "test_run_daemon.py"
Cohesion: 0.17
Nodes (17): main(), Continuous daemon runner for research_scanner. Runs fetch -> process requests…, Executes a single cycle of the research_scanner pipeline: 1. Scan cycle…, Executes the research_scanner pipeline continuously every interval_seconds.…, run_continuous_daemon(), run_single_cycle(), Unit tests for run_daemon.py module., Verify full daemon loop executes steps 1-4 in sequence. (+9 more)
...
### Community 167 - "get_curator_prompt"
Cohesion: 0.50
Nodes (4): get_curator_prompt(), Constructs the Curator prompt by loading the base prompt template, fetching…, Verify curator prompt retrieval with embedded unconsumed items and priorities., test_get_curator_prompt()
...
- **Why does `init_db()` connect `init_db` to `config.py`, `test_scoring.py`, `test_run_daemon.py`, `export_to_obsidian.py`, `db.py`, `test_log_trade.py`, `scan.py`, `test_export_to_obsidian.py`, `test_thesis_ledger.py`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `BacktestEngine` connect `BacktestEngine` to `bootstrap_model.py`, `engine.py`, `PortfolioRanker`, `ChronosInference`, `EnsembleAgent`, `RegimeDetector`, `GovernanceEngine`, `VolatilityGuard`, `test_leakage.py`, `trading_system/main.py`?**
