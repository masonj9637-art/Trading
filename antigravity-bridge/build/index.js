...
function runCommand(file, args, timeoutMs) {
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            clearTimeout(timer);
            if (timedOut) {
                const err = new Error(`Execution timed out after ${timeoutMs / 1000} seconds.`);
                err.code = "ETIMEDOUT";
                err.killed = true;
                err.stdout = stdout;
                err.stderr = stderr;
 ...
 */
function isQuotaError(stderr = "", stdout = "", extra = "") {
...
const RUN_AGENT_TOOL = {
    ...
    description: "Executes an autonomous agent via the local agy CLI binary in headless mode. " +
    ...
    inputSchema: {
        ...
        properties: {
            prompt: {
                type: "string",
                description: "Required. The complete, fully self-contained task prompt text for the agent to execute.",
            },
            timeout_seconds: {
                type: "number",
                description: "Optional. Timeout limit in seconds for the agent call (default: 300 seconds). Execution will be aborted if exceeded.",
            },
        },
        required: ["prompt"],
    },
...
const CHECK_STATUS_TOOL = {
    ...
    description: "Lightweight check to verify that the local agy CLI binary is installed and executable without consuming API quota. " +
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        auth_status_note: "agy CLI does not provide a zero-quota auth-status endpoint. Binary presence and settings file confirmed.",
                    }, null, 2),
                },
            ],
        };
...
async function handleRunAgent(prompt, timeoutSeconds) {
